...
from src.streamlit_utils import clear_scanner_candidate_summary_cache, load_scanner_candidate_summary_cached
from src.suggestions_page_state import build_scanner_research_debug_entry
from src.suggestions_service import SuggestionPayload, apply_suggestion, create_suggestion, list_suggestions, review_suggestion
...
class TestScannerAudit(unittest.TestCase):
    ...
    def test_new_daily_export_for_same_ticker_increases_recurrence_metrics(self):
        conn = self._conn()
        tmp = self._tmpdir()
        (tmp / "Export_2026-03-15.csv").write_text("Ticker,Note\nAAOI,one\n", encoding="utf-8")
        (tmp / "Export_2026-03-16.csv").write_text("Ticker,Note\nAAOI,one\n", encoding="utf-8")
        ...
        self.assertEqual(int(row["observed_days"]), 2)
        self.assertEqual(int(row["observations_last_5d"]), 2)
        self.assertEqual(int(row["observations_last_10d"]), 2)
        self.assertEqual(int(row["current_streak"]), 2)
        self.assertEqual(str(row["last_seen"]).split(" ")[0], "2026-03-16")
        conn.close()

    def test_processed_file_results_include_observed_date_diagnostics(self):
        conn = self._conn()
        tmp = self._tmpdir()
        path = tmp / "Export_2026-03-16.csv"
        path.write_text("Ticker,Note\nAAOI,one\n", encoding="utf-8")

        result = import_tc2000_exports(conn, folder=tmp, pattern="*.csv", default_source_label="tc2000_export")
        file_result = result["file_results"][0]

        self.assertEqual(file_result["observed_date_min"], "2026-03-16")
        self.assertEqual(file_result["observed_date_max"], "2026-03-16")
        self.assertEqual(int(file_result["observed_dates_count"]), 1)
        conn.close()

    def test_cached_scanner_summary_can_be_explicitly_refreshed_after_new_hit(self):
        tmp = self._tmpdir()
        db_path = tmp / "scanner_cache_test.duckdb"
        with patch("src.database.DB_PATH", db_path), patch("src.streamlit_utils.DB_PATH", db_path):
            ...
            conn.execute(
                ...
                (1, 'tc2000', 'AAOI', 'AAOI', '2026-03-15', '2026-03-15 08:00:00', 'f1.csv', 'tc2000_export', 'tc2000_export', 'aaoi-1')
                """
            )
            conn.close()

            clear_scanner_candidate_summary_cache()
            first = load_scanner_candidate_summary_cached(("test-db", 1))
            conn = duckdb.connect(str(db_path))
            conn.execute(
                """
                insert into scanner_hit_history(
                ...
                (2, 'tc2000', 'AAOI', 'AAOI', '2026-03-16', '2026-03-16 08:00:00', 'f2.csv', 'tc2000_export', 'tc2000_export', 'aaoi-2')
                """
            )
            conn.close()

            cached_again = load_scanner_candidate_summary_cached(("test-db", 1))
            clear_scanner_candidate_summary_cache()
            refreshed = load_scanner_candidate_summary_cached(("test-db", 1))

        self.assertEqual(int(first[first["ticker"] == "AAOI"].iloc[0]["observed_days"]), 1)
        self.assertEqual(int(cached_again[cached_again["ticker"] == "AAOI"].iloc[0]["observed_days"]), 1)
        self.assertEqual(int(refreshed[refreshed["ticker"] == "AAOI"].iloc[0]["observed_days"]), 2)
        clear_scanner_candidate_summary_cache()
    ...
    @patch(
        ...
        return_value={
            "ticker": "SEZL",
            "company_name": "Sezzle",
            "description": "Provides installment payments and digital checkout financing products.",
            "sic_description": "Financial Services",
        },
    )
    def test_queue_listing_exposes_structured_scanner_audit_theme_selections(self, _mock_profile, _mock_key):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'Fintech - Payments', 'Fintech', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (2, 'Fintech - Lending', 'Fintech', true)")
        conn.execute(
            """
            insert into scanner_hit_history(
                import_run_id, import_source, normalized_ticker, raw_ticker, observed_date, observed_at,
            ...
            (1, 'tc2000', 'SEZL', 'SEZL', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'sezl-queue-1')
            """
        )
        ...
        promote_scanner_candidate_to_theme_review(
            conn,
            "SEZL",
            "Interesting BNPL name",
            custom_existing_theme_ids=[1, 2],
        )
        queue = list_suggestions(conn, status="pending")

        row = queue[queue["proposed_ticker"] == "SEZL"].iloc[0]
        self.assertEqual(
            set(str(row["selected_existing_theme_names"]).split(", ")),
            {"Fintech - Payments", "Fintech - Lending"},
        )
        self.assertEqual(row["promotion_note"], "Interesting BNPL name")
        conn.close()
    ...
    def test_applying_scanner_audit_review_theme_uses_structured_selected_existing_themes(self, _mock_profile, _mock_key):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'Fintech - Payments', 'Fintech', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (2, 'Fintech - Lending', 'Fintech', true)")
        conn.execute(
            ...
            (1, 'tc2000', 'SEZL', 'SEZL', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'sezl-apply-1')
            """
        )

        promoted = promote_scanner_candidate_to_theme_review(
            conn,
            "SEZL",
            "Interesting BNPL name",
            custom_existing_theme_ids=[1, 2],
        )
        review_suggestion(conn, int(promoted["suggestion_id"]), "approved", "approve selected themes")
        apply_suggestion(conn, int(promoted["suggestion_id"]), "apply approved review")

        memberships = conn.execute(
            "select theme_id, ticker from theme_membership where ticker = 'SEZL' order by theme_id"
        ).fetchall()
        self.assertEqual(memberships, [(1, "SEZL"), (2, "SEZL")])
        status = conn.execute("select status from theme_suggestions where suggestion_id = ?", [int(promoted["suggestion_id"])]).fetchone()[0]
        self.assertEqual(status, "applied")
        conn.close()

    def test_direct_apply_from_scanner_audit_writes_membership_and_preserves_audit_trail(self):
        conn = self._conn()
        conn.execute(
            """
            insert into themes(id, name, category, is_active) values
            (1, 'Fintech - Payments', 'Fintech', true),
            (2, 'Fintech - Lending', 'Fintech', true)
            """
        )
        conn.execute(
            """
            ...
            (1, 'tc2000', 'SEZL', 'SEZL', '2026-03-16', '2026-03-16 08:00:00', 'f1.csv', 'tc2000_export', 'tc2000_export', 'sezl-1')
            """
        )
        draft = {
            "ticker": "SEZL",
            "company_name": "Sezzle",
            "short_company_description": "Buy-now-pay-later payments platform.",
            "possible_similar_tickers": ["AFRM"],
            "suggested_existing_themes": [],
            "possible_new_theme": None,
            "confidence": "medium",
            "rationale": "Payments and lending adjacency.",
            "caveats": "",
            "recommended_action": "add_to_existing_theme_review",
        ...
        result = apply_scanner_candidate_selected_themes(
            conn,
            "SEZL",
            "Strong Scanner Audit conviction",
            research_draft=draft,
            custom_existing_theme_ids=[1, 2],
            custom_new_themes=["Consumer BNPL"],
        )
        ...
        suggestion = conn.execute(
            """
            select status, source, reviewer_notes, source_context_json
            from theme_suggestions
            where suggestion_id = ?
            """,
        ...
        onboarding = conn.execute(
            """
            select onboarding_source, history_readiness_status, backfill_status
        ...
        self.assertEqual(memberships, [(1, "SEZL"), (2, "SEZL")])
        self.assertEqual(str(suggestion[0]), "applied")
        self.assertEqual(str(suggestion[1]), "scanner_audit")
        self.assertIn("directly from Scanner Audit", str(suggestion[2]))
        self.assertIn('"custom_new_themes": ["Consumer BNPL"]', str(suggestion[3]))
        self.assertEqual(str(onboarding[0]), "suggestion:scanner_audit")
        self.assertEqual(str(onboarding[1]), "needs_backfill")
        self.assertEqual(str(onboarding[2]), "needed")
        conn.close()

    def test_legacy_manual_add_ticker_suggestion_apply_still_works(self):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI', 'Tech', true)")
        suggestion_id = create_suggestion(
            conn,
            SuggestionPayload(
                suggestion_type="add_ticker_to_theme",
                source="manual",
                proposed_ticker="NVDA",
                existing_theme_id=1,
            ),
        )
        review_suggestion(conn, int(suggestion_id), "approved", "looks good")
        apply_suggestion(conn, int(suggestion_id), "apply manual add")

        memberships = conn.execute("select theme_id, ticker from theme_membership").fetchall()
        self.assertEqual(memberships, [(1, "NVDA")])
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "company_name": "Applied Optoelectronics",
            "short_company_description": "Optical transceiver and fiber interconnect supplier for data-center and telecom networks.",
            "possible_similar_tickers": ["LITE", "CIEN"],
            "suggested_existing_themes": [
                {"theme_id": 1, "theme_name": "AI - Infrastructure", "why_it_might_fit": "Adjacent data-center demand exposure."},
                {"theme_id": 2, "theme_name": "AI - Edge Computing", "why_it_might_fit": "Broad communications infrastructure adjacency."},
            ],
            "possible_new_theme": "None suggested",
            "confidence": "low",
            "rationale": "The company appears to serve data-center and telecom end markets through optical networking and interconnect products. AI - Infrastructure and AI - Edge Computing are weaker adjacent fits because they describe end-market exposure rather than the company's actual role in the stack, and a narrower direct role would be more precise than the current governed taxonomy.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_placeholder_none_suggested_does_not_block_narrow_new_theme_override(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI - Infrastructure', 'Tech', true)")
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'AAOI', 'AAOI', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'aaoi-openai-none-suggested')
            """
        )
        ...
        self.assertNotEqual(str(draft["possible_new_theme"]).lower(), "none suggested")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "AAOIX",
            "company_name": "Applied Optoelectronics",
            "description": "Provides communication equipment products for network operators and data-center customers.",
            "sic_description": "Communications Equipment",
        },
    )
    @patch(
        ...
        return_value={
            "company_name": "Applied Optoelectronics",
            "short_company_description": "Supplier of optical networking transceivers and fiber interconnect products for data-center and telecom deployments.",
            "possible_similar_tickers": ["LITE", "CIEN"],
            "suggested_existing_themes": [
                {"theme_id": 1, "theme_name": "AI - Infrastructure", "why_it_might_fit": "Broader data-center adjacency."},
                {"theme_id": 2, "theme_name": "AI - Edge Computing", "why_it_might_fit": "Communications adjacency."},
    ...
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_can_promote_narrow_new_theme_from_ai_role_context_even_when_profile_text_is_generic(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI - Infrastructure', 'Tech', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (2, 'AI - Edge Computing', 'Tech', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (1, 'NVDA')")
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'AAOIX', 'AAOIX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'aaoix-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "AAOIX")

        self.assertEqual(draft["research_mode"], "openai")
        self.assertEqual(draft["recommended_action"], "consider_new_theme")
        self.assertIn(draft["possible_new_theme"], {"Optical Networking", "Data Center Optics", "Optical Interconnects", "AI Fiber Optics"})
        trace = draft.get("research_decision_trace") or {}
        self.assertTrue(trace.get("should_promote_new_theme"))
        self.assertIn("optical_networking", trace.get("candidate_roles_detected") or [])
        conn.close()
    ...
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_decision_surface_keeps_proposed_new_theme_and_adjacent_existing_themes(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI - Infrastructure', 'Tech', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (2, 'AI - Edge Computing', 'Tech', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (1, 'NVDA')")
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'AAOIZ', 'AAOIZ', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'aaoiz-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "AAOIZ")

        self.assertEqual(draft["recommended_action"], "consider_new_theme")
        self.assertIn(draft["possible_new_theme"], {"Optical Networking", "Data Center Optics", "Optical Interconnects", "AI Fiber Optics"})
        self.assertLessEqual(len(draft["suggested_existing_themes"]), 3)
        self.assertEqual(
            [item["fit_label"] for item in draft["suggested_existing_themes"][:2]],
    ...
    @patch(
        ...
        return_value={
            "company_name": "AXT Inc.",
            "short_company_description": "Supplier of compound semiconductor substrate materials.",
            "possible_similar_tickers": ["WOLF"],
            "suggested_existing_themes": [
                {"theme_id": 3, "theme_name": "Chemicals - Diversified", "why_it_might_fit": "Broad materials exposure."},
                {"theme_id": 4, "theme_name": "Coatings & Paints", "why_it_might_fit": "Loose materials adjacency."},
            ],
            "possible_new_theme": "",
            "confidence": "low",
            "rationale": "The company provides semiconductor materials and substrates into communications and sensing markets. Chemicals - Diversified and Coatings & Paints are broader fit buckets rather than precise descriptions of what the company actually provides.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_decision_surface_keeps_proposed_new_theme_and_broad_existing_materials_themes(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (3, 'Chemicals - Diversified', 'Materials', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (4, 'Coatings & Paints', 'Materials', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (3, 'SHW')")
        conn.execute("insert into theme_membership(theme_id, ticker) values (4, 'PPG')")
        conn.execute(
            """
            insert into scanner_hit_history(
                import_run_id, import_source, normalized_ticker, raw_ticker, observed_date, observed_at,
            ...
            (1, 'tc2000', 'AXTY', 'AXTY', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'axty-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "AXTY")
    ...
    @patch(
        ...
        return_value={
            "ticker": "DAVEZ",
            "company_name": "Dave Example",
            "description": "Provides branch and ATM software for regional banking institutions.",
            "sic_description": "Business Services",
        },
    )
    def test_weak_fit_cases_remain_conservative_and_do_not_emit_vague_new_theme_labels(self, _mock_profile, _mock_key):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI - Infrastructure', 'Tech', true)")
        conn.execute(
            """
            ...
            (1, 'tc2000', 'DAVEZ', 'DAVEZ', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'davez-weak-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "DAVEZ")
    ...
    @patch(
        ...
        return_value={
            ...
            "description": "Supplies optical transceivers and interconnect modules for data-center communications infrastructure.",
            "sic_description": "Communications Equipment",
        },
    )
    @patch(
        ...
        return_value={
            "company_name": "Role Link",
            "short_company_description": "Communications hardware supplier.",
            "possible_similar_tickers": ["NVDA"],
            "suggested_existing_themes": [
                {"theme_id": 4, "theme_name": "AI - Infrastructure", "why_it_might_fit": "Data-center adjacency."}
            ],
            "possible_new_theme": "",
            "confidence": "low",
            "rationale": "The company provides optical networking and interconnect products. AI - Infrastructure is a weaker end-market adjacency fit, while the direct business role is more precise than the current governed taxonomy.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_merge_promotes_narrow_new_theme_when_rationale_identifies_precision_gap(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (4, 'AI - Infrastructure', 'Tech', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (4, 'NVDA')")
        conn.execute(
            """
            insert into scanner_hit_history(
                import_run_id, import_source, normalized_ticker, raw_ticker, observed_date, observed_at,
            ...
            (1, 'tc2000', 'ROLELINK', 'ROLELINK', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'rolelink-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "ROLELINK")
    ...
    @patch(
        ...
        return_value={
            "company_name": "Role Materials",
            "short_company_description": "Materials supplier serving semiconductor applications.",
            "possible_similar_tickers": ["SHW"],
            "suggested_existing_themes": [
                {"theme_id": 5, "theme_name": "Chemicals - Diversified", "why_it_might_fit": "Broad materials adjacency."}
            ],
            "possible_new_theme": "",
            "confidence": "low",
            "rationale": "The company provides semiconductor materials and substrates. Chemicals - Diversified is only an adjacent fit, and the direct role is more specific than the current governed taxonomy.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_merge_surfaces_semiconductor_materials_new_theme_from_adjacent_only_output(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (5, 'Chemicals - Diversified', 'Materials', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (5, 'SHW')")
        conn.execute(
            """
            insert into scanner_hit_history(
                import_run_id, import_source, normalized_ticker, raw_ticker, observed_date, observed_at,
            ...
            (1, 'tc2000', 'ROLEMAT', 'ROLEMAT', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'rolemat-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "ROLEMAT")
        ...
        self.assertTrue(draft["suggested_existing_themes"])
        self.assertIn("adjacent", draft["rationale"].lower())
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "DAVEY",
            "company_name": "Davey Example",
            "description": "Provides branch and ATM banking software for regional financial institutions.",
            "sic_description": "Business Services",
        },
    )
    @patch(
        ...
        return_value={
            "company_name": "Davey Example",
            "short_company_description": "Software provider for bank operations.",
            "possible_similar_tickers": [],
            "suggested_existing_themes": [],
            "possible_new_theme": "",
            "confidence": "low",
            "rationale": "No strong governed-theme fit was identified from the available profile and taxonomy context.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_merge_keeps_conservative_no_fit_behavior_without_precision_gap(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (6, 'Space Infrastructure', 'Industrial', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (6, 'RKLB')")
        conn.execute(
            """
            insert into scanner_hit_history(
                import_run_id, import_source, normalized_ticker, raw_ticker, observed_date, observed_at,
            ...
            (1, 'tc2000', 'DAVEY', 'DAVEY', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'davey-openai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "DAVEY")
    ...
    def test_build_scanner_research_debug_entry_preserves_forced_regeneration_source_on_rerender(self):
        draft = {
            "ticker": "AAOI",
            "generated_at": "2026-03-16 09:15:00",
            "research_mode": "openai",
            "draft_source": "forced_regeneration",
        }

        first = build_scanner_research_debug_entry("AAOI", draft, draft_source="forced_regeneration")
        rerendered = build_scanner_research_debug_entry("AAOI", draft, prior_debug=first)

        self.assertEqual(first["draft_source"], "forced_regeneration")
        self.assertEqual(rerendered["draft_source"], "forced_regeneration")
        self.assertEqual(rerendered["generated_at"], "2026-03-16 09:15:00")
    ...
    def test_forced_regeneration_reuses_last_good_profile_context_when_live_lookup_degrades(self, mock_profile, _mock_key):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'AI - Infrastructure', 'Tech', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (1, 'NVDA')")
        conn.execute(
            ...
            (1, 'tc2000', 'AAOI', 'AAOI', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'aaoi-profile-cache-1')
            """
        )

        first, reused_first = get_or_create_scanner_research_draft(conn, "AAOI")
        refreshed, reused_second = get_or_create_scanner_research_draft(
            conn,
            "AAOI",
            existing_draft=first,
        ...
        self.assertEqual(first["company_name"], "Applied Optoelectronics")
        self.assertIn("optical transceivers", first["short_company_description"].lower())
        self.assertEqual(refreshed["company_name"], "Applied Optoelectronics")
        self.assertIn("optical transceivers", refreshed["short_company_description"].lower())
        self.assertNotEqual(refreshed["short_company_description"], "No verified company description available.")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "QNTY",
            "company_name": "Qnity Electronics",
            "description": "Supplies specialty electronics materials, semiconductor packaging materials, and related substrate products for semiconductor and electronics customers.",
            "sic_description": "Electronic Materials",
        },
    )
    def test_fallback_electronics_materials_suggestions_suppress_healthcare_junk(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Semiconductor Materials", "Semiconductors", "WOLF"),
            (2, "Specialty Chemicals", "Materials", "SHW"),
            (3, "Animal Health", "Healthcare", "ZTS"),
            (4, "Dental", "Healthcare", "XRAY"),
            (5, "CRO & CMO", "Healthcare", "IQV"),
        ]:
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'QNTY', 'QNTY', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'qnty-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "QNTY")
        ...
        self.assertNotIn("CRO & CMO", suggested_names)
        self.assertIn(
            draft["possible_new_theme"],
            {None, "Semiconductor Materials", "Semiconductor Substrates", "Electronics Materials", "Specialty Semiconductor Materials"},
        )
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "YOUX",
            "company_name": "Clear Secure",
            "description": "Provides secure identity verification and biometric member authentication services for airport and venue access.",
            "sic_description": "Identity Verification Services",
        },
    )
    def test_fallback_identity_archetype_suppresses_optical_networking_misclassification(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'YOUX', 'YOUX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'youx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "YOUX")
        ...
        self.assertNotIn(draft["possible_new_theme"], {"Optical Networking", "Data Center Optics", "Optical Interconnects"})
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "SEZLX",
            "company_name": "Sezzle",
            "description": "Provides installment payments, digital checkout, and consumer lending products for merchants and shoppers.",
            "sic_description": "Financial Services",
        },
    )
    def test_fallback_fintech_archetype_beats_cloud_devops_primary_match(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'SEZLX', 'SEZLX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'sezlx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "SEZLX")
        top_names = [item["theme_name"] for item in draft["suggested_existing_themes"][:2]]

        self.assertIn(top_names[0], {"Fintech - Payments", "Fintech - Lending"})
        self.assertNotEqual(top_names[0], "Cloud DevOps")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "NBISX",
            "company_name": "NextBase Infrastructure",
            "description": "Builds and operates hyperscale AI data-center capacity, power-dense server campuses, and GPU cluster infrastructure.",
            "sic_description": "Data Center Infrastructure",
        },
    )
    def test_fallback_ai_data_center_archetype_surfaces_data_center_theme(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "AI - Data Centers", "AI", "VRT"),
            (2, "Cloud Software", "Software", "NOW"),
        ]:
            conn.execute("insert into themes(id, name, category, is_active) values (?, ?, ?, true)", [theme_id, theme_name, category])
            conn.execute("insert into theme_membership(theme_id, ticker) values (?, ?)", [theme_id, ticker])
        conn.execute(
            ...
            (1, 'tc2000', 'NBISX', 'NBISX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'nbisx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "NBISX")

        self.assertTrue(draft["suggested_existing_themes"])
        self.assertEqual(draft["suggested_existing_themes"][0]["theme_name"], "AI - Data Centers")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "MATIX",
            "company_name": "Material Interconnect",
            "description": "Produces semiconductor packaging materials and specialty electronics materials used in advanced chip packaging and semiconductor manufacturing. The company also sells device interconnect materials.",
            "sic_description": "Electronic Materials",
        },
    )
    def test_optical_or_interconnect_wording_does_not_override_stronger_materials_context(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Semiconductor Materials", "Semiconductors", "WOLF"),
            (2, "Optical Networking", "Communications", "LITE"),
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'MATIX', 'MATIX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'matix-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "MATIX")
    ...
    @patch(
        ...
        return_value={
            "ticker": "POETX",
            "company_name": "Photon Example Technologies",
            "description": "Develops optical engines, light source products, custom optical modules, and optical interposer integration for AI systems and hyperscale data centers using wafer-level semiconductor manufacturing techniques.",
            "sic_description": "Photonic Integration",
        },
    )
    def test_poet_like_role_evidence_does_not_let_generic_growth_theme_outrank_role_specific_options(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Growth - High", "Style", "NVDA"),
            (2, "Optical Networking", "Communications", "LITE"),
        ...
        conn.execute(
            ...
            (1, 'tc2000', 'POETX', 'POETX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'poetx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "POETX")
        suggested_names = [item["theme_name"] for item in draft["suggested_existing_themes"]]

        self.assertTrue(suggested_names)
        self.assertNotEqual(suggested_names[0], "Growth - High")
        self.assertIn("Optical Networking", suggested_names)
        if draft["possible_new_theme"] is not None:
            self.assertIn(draft["possible_new_theme"], {"Optical Networking", "Data Center Optics", "Optical Interconnects", "AI Fiber Optics"})
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "ROLEGX",
            "company_name": "Role Growth Example",
            "description": "Provides optical interposer modules, photonic integration, and custom optical engines for hyperscale AI and data-center networking deployments.",
            "sic_description": "Photonic Components",
        },
    )
    def test_strong_role_evidence_suppresses_generic_factor_style_fallback_themes(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Growth - High", "Style", "NVDA"),
            (2, "Momentum Leaders", "Factor", "SMCI"),
            (3, "AI - Infrastructure", "AI", "VRT"),
        ]:
            conn.execute("insert into themes(id, name, category, is_active) values (?, ?, ?, true)", [theme_id, theme_name, category])
            conn.execute("insert into theme_membership(theme_id, ticker) values (?, ?)", [theme_id, ticker])
        conn.execute(
            ...
            (1, 'tc2000', 'ROLEGX', 'ROLEGX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'rolegx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "ROLEGX")
        top_names = [item["theme_name"] for item in draft["suggested_existing_themes"][:2]]

        self.assertNotIn("Growth - High", top_names[:1])
        self.assertNotIn("Momentum Leaders", top_names[:1])
        self.assertEqual(draft["recommended_action"], "consider_new_theme")
        self.assertIsNotNone(draft["possible_new_theme"])
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "POETAI",
            "company_name": "Photon Example Technologies",
            "description": "Develops photonic engines and optical interposer products for hyperscale AI systems and data-center optical modules.",
            "sic_description": "Photonic Integration",
        },
    )
    @patch(
        ...
        return_value={
            "company_name": "Photon Example Technologies",
            "short_company_description": "Photonic engines and optical interposer products for AI systems.",
            "possible_similar_tickers": ["LITE"],
            "suggested_existing_themes": [
                {"theme_id": 1, "theme_name": "Growth - High", "why_it_might_fit": "Broad growth exposure."},
                {"theme_id": 2, "theme_name": "AI - Infrastructure", "why_it_might_fit": "Adjacent AI data-center exposure."},
            ],
            "possible_new_theme": "",
            "confidence": "medium",
            "rationale": "The company provides optical engines and optical interposer products into hyperscale AI systems and data-center deployments. Growth - High is a broad style bucket rather than a direct description of what the company provides.",
            "caveats": ["Advisory only."],
            "recommended_action": "watch_only",
        },
    )
    @patch("src.scanner_research.openai_api_key", return_value="test-key")
    def test_openai_role_specific_candidate_new_theme_survives_generic_growth_existing_theme(self, _mock_key, _mock_call, _mock_profile):
        conn = self._conn()
        conn.execute("insert into themes(id, name, category, is_active) values (1, 'Growth - High', 'Style', true)")
        conn.execute("insert into themes(id, name, category, is_active) values (2, 'AI - Infrastructure', 'AI', true)")
        conn.execute("insert into theme_membership(theme_id, ticker) values (1, 'NVDA')")
        conn.execute("insert into theme_membership(theme_id, ticker) values (2, 'VRT')")
        conn.execute(
            """
            ...
            (1, 'tc2000', 'POETAI', 'POETAI', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'poetai-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "POETAI")

        self.assertEqual(draft["recommended_action"], "consider_new_theme")
        self.assertIn(draft["possible_new_theme"], {"Optical Networking", "Data Center Optics", "Optical Interconnects", "AI Fiber Optics"})
        self.assertNotEqual(draft["suggested_existing_themes"][0]["theme_name"], "Growth - High")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "KRMNX",
            "company_name": "Karman Defense Systems",
            "description": "Designs and manufactures defense and space systems, satellite payload structures, and mission hardware for aerospace and government programs.",
            "sic_description": "Aerospace Systems",
        },
    )
    def test_defense_space_systems_archetype_suppresses_ai_server_misclassification(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Defense Tech", "Industrial", "LMT"),
            (2, "Space Infrastructure", "Industrial", "RKLB"),
            (3, "AI Server Systems", "AI", "SMCI"),
        ]:
            conn.execute("insert into themes(id, name, category, is_active) values (?, ?, ?, true)", [theme_id, theme_name, category])
            conn.execute("insert into theme_membership(theme_id, ticker) values (?, ?)", [theme_id, ticker])
        conn.execute(
            ...
            (1, 'tc2000', 'KRMNX', 'KRMNX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'krmnx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "KRMNX")
        top_names = [item["theme_name"] for item in draft["suggested_existing_themes"][:2]]

        self.assertIn(top_names[0], {"Defense Tech", "Space Infrastructure"})
        self.assertNotEqual(top_names[0], "AI Server Systems")
        conn.close()
    ...
    @patch(
        ...
        return_value={
            "ticker": "ENABX",
            "company_name": "Enable Materials",
            "description": "Supplies advanced electronics materials and component substrates for communications systems. The company mentions interconnect and systems integration but generates revenue from materials and packaging inputs.",
            "sic_description": "Electronic Materials",
        },
    )
    def test_isolated_interconnect_and_systems_terms_do_not_override_dominant_materials_role(self, _mock_profile, _mock_key):
        conn = self._conn()
        for theme_id, theme_name, category, ticker in [
            (1, "Semiconductor Materials", "Semiconductors", "WOLF"),
            (2, "AI Server Systems", "AI", "SMCI"),
            (3, "Optical Networking", "Communications", "LITE"),
        ]:
            conn.execute("insert into themes(id, name, category, is_active) values (?, ?, ?, true)", [theme_id, theme_name, category])
            conn.execute("insert into theme_membership(theme_id, ticker) values (?, ?)", [theme_id, ticker])
        conn.execute(
            ...
            (1, 'tc2000', 'ENABX', 'ENABX', '2026-03-12', '2026-03-12 08:00:00', 'f1.csv', 'tc2000', 'Momentum', 'enabx-1')
            """
        )

        draft = generate_scanner_research_draft(conn, "ENABX")

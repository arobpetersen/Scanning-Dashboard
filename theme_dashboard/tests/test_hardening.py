...
class TestManualTickerSuppression(unittest.TestCase):
    ...
    def test_themes_page_source_explains_selected_history_provenance_and_current_metric_semantics(self):
        ...
        self.assertIn('_render_summary_metric("Rank History", rank_history_context)', content)
        self.assertIn("canonical_theme_history_window(conn, 7)", content)
        self.assertIn("canonical_theme_history_window(conn, 30)", content)
